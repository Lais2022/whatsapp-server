...
const setMessageStatus = (id, status) => {
  if (!id || !status) return;
  messageStatusMap.set(id, { status, updatedAt: Date.now() });

  if (messageStatusMap.size > MESSAGE_STATUS_MAX) {
    const firstKey = messageStatusMap.keys().next().value;
    messageStatusMap.delete(firstKey);
  }
};
...
const postWebhook = async (events) => {
  ...
  try {
    ...
    if (!res.ok) console.log(`[WEBHOOK] falhou status=${res.status}`);
  } catch (err) {
...
// Controle de loop de desconexão
...
const loadKnownContactJids = () => {
  try {
    ...
    return new Map(Object.entries(parsed).filter(([, jid]) => typeof jid === 'string'));
  } catch {
    return new Map();
  }
};

const saveKnownContactJids = () => {
  try {
    fs.writeFileSync(CONTACT_JIDS_FILE, JSON.stringify(Object.fromEntries(knownContactJids), null, 2));
  } catch (err) {
    log(LogLevel.WARN, 'Não foi possível salvar mapa de contatos', { error: err.message });
  }
};
...
const normalizeJid = (recipient) => {
  ...
  if (raw.includes('@')) {
    ...
    return knownContactJids.get(bare) || raw;
  }
...
const maskRecipient = (recipient) => String(recipient || '').substring(0, 12);
...
const checkRateLimit = (phone) => {
  const now = Date.now();
  const key = String(phone || '').includes('@') ? String(phone).trim() : formatPhone(phone);
...
function trackDisconnect(statusCode) {
  const now = Date.now();
  const withinWindow = now - lastDisconnectAt < 60000;
  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        updateState({
          isConnected: false,
          isReady: wasLoggedOut ? false : state.isReady,
          isAuthenticated: false,
          isConnecting: false,
          qrAvailable: false,
          status: 'disconnected',
    ...
    sock.ev.on('messages.update', (updates) => {
      for (const u of updates) {
        try {
          ...
          if (status === 4) mapped = 'delivered';
          else if (status === 5 || status === 6) mapped = 'read';
          else if (status === 1) mapped = 'failed';
...
app.get('/message-status', (req, res) => {
  ...
  for (const id of ids) {
    const entry = messageStatusMap.get(id);
...
app.get('/diagnostics', (req, res) => {
  res.json({
    ...
    metrics: {
      ...
      uptime: Math.floor((Date.now() - metrics.startTime) / 1000),
      avgLatency:
        metrics.sendLatencies.length > 0
...
async function sendWithIsolation(operation, timeout) {
  ...
  try {
    const result = await Promise.race([
      operation(),
      new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout de envio')), timeout)),
    ]);
...
const handleSendText = async (req, res) => {
  const phone = req.body.jid || req.body.to || req.body.phone || req.body.recipient || req.body.number;
  ...
  if (success) {
    log(LogLevel.INFO, 'Texto enviado', {
      requested: maskRecipient(phone),
      to: maskRecipient(jid),
...
app.post('/send-image', async (req, res) => {
  ...
  if (success) {
    log(LogLevel.INFO, 'Imagem enviada', {
      to: maskRecipient(jid),
      messageId: result?.key?.id,
      latency,
    });
...
app.post('/send-audio', async (req, res) => {
  ...
  log(LogLevel.DEBUG, 'Processando áudio', {
    mimetype: receivedMimetype,
    ptt,
    size: audioData?.length?.toString().slice(0, 5) + '...',
  });
  ...
  const finalMimetype = req.body.mimetype || 'audio/ogg; codecs=opus';
  ...
  if (success) {
    log(LogLevel.INFO, 'Áudio enviado', {
      to: maskRecipient(jid),
      messageId: result?.key?.id,
      mimetype: finalMimetype,
      latency,
  ...
  log(LogLevel.ERROR, 'Falha ao enviar áudio', {
    to: maskRecipient(jid),
    error,
    mimetype: finalMimetype,
  });
...
app.post('/send-video', async (req, res) => {
  ...
  if (success) {
    log(LogLevel.INFO, 'Vídeo enviado', {
      to: maskRecipient(jid),
      messageId: result?.key?.id,
      latency,
    });
...
app.post('/send-document', async (req, res) => {
  ...
  if (success) {
    log(LogLevel.INFO, 'Documento enviado', {
      to: maskRecipient(jid),
      messageId: result?.key?.id,
      latency,
    });
...
// ============================================
app.listen(PORT, () => {
  log(LogLevel.INFO, `Servidor WhatsApp v${SERVER_VERSION} rodando`, {
    port: PORT,
